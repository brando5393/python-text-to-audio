  ...
  def __init__(self, app_log_display):
    ...
    if os.path.isfile(self.log_file_location):
      ...
      try:
        ...
        self.app_log_display.insert("end",f"[{datetime.datetime.now()}]: WARN | Unable to create log file at locaton {self.log_file_location}")

  def add_event(self,status,msg,err):
    """Add an event to the application log file"""
    with open(self.log_file_location, "a") as log_file:
      log_file.write(f"[{datetime.datetime.now()}]: {status.upper().strip()} | {msg.strip()} | {err.strip()}\n")
